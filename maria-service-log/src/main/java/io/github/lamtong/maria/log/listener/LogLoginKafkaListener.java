...
/**
 * {@code Kafka} 登录认证日志监听器, 监听并消费登录认证日志.
 *
 * @author Lam Tong
 * @version 0.0.1
 * @since 0.0.1
...
@Component
public class LogLoginKafkaListener {

    private static final Logger logger = LoggerFactory.getLogger(LogLoginKafkaListener.class);

    @Autowired
    private LogLoginService logLoginService;

    /**
     * 监听主题 {@link ServiceLog#KAFKA_TOPIC_LOG_LOGIN} 队列并批量消费.
     *
     * @param records 批量登录认证消息
     */
    @SuppressWarnings(value = {"DuplicatedCode"})
    @KafkaListener(id = ServiceLog.KAFKA_LISTENER_LOG_LOGIN_ID, groupId = ServiceLog.KAFKA_LISTENER_LOG_LOGIN_GROUP,
            errorHandler = "consumerAwareListenerErrorHandler", topics = {ServiceLog.KAFKA_TOPIC_LOG_LOGIN})
    public void onMessage(List<ConsumerRecord<String, String>> records) {
        ...
        for (ConsumerRecord<String, String> record : records) {
            ...
            LogLogin log = JSON.parseObject(record.value(), LogLogin.class);
