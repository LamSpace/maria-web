...
/**
 * 接收示例消息, 向前端推送消息时默认按照广播处理. 主要是演示 {@code RocketMQ} 的使用.
 *
 * @author Lam Tong
 * @version 0.0.1
 * @since 0.0.1
...
@Service
@RocketMQMessageListener(consumerGroup = ServiceRocket.GROUP_DEMO,
        topic = ServiceRocket.TOPIC_DEMO,
        consumeMode = ConsumeMode.CONCURRENTLY,
        messageModel = MessageModel.CLUSTERING)
public class RocketMQListenerImpl implements RocketMQListener<MariaMessage> {
    ...
    public void onMessage(MariaMessage message) {
        ...
        if (message.isBroadcast()) {
            ...
            WebSocketHandler.unicast(message.getUsername(), message.getMessage());
        }
    }
